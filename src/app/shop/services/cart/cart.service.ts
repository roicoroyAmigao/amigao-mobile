...
export class CartService {
  ...
  increaseCartQty(index) {
    ...
    const increasedQty = this.items[index].cartQuantity + 1;
  ...
  decreaseCartQty(index) {
    ...
    const decreasedQty = this.items[index].cartQuantity - 1;
  ...
  addLocalCartItems() {
    ...
    const index = this.items.length > 0 ? this.items.findIndex(value => value.id === this.item.id) : -1;
